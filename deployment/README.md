...
The channel also provides `python-snakemake-executor-plugin-guix-ssh`.  It
keeps the DAG and `.snakemake` metadata on the controller, copies immutable
Guix closures with `guix copy`, stages job files with `rsync`, and only reports
success after outputs have been published locally.

An executor profile can contain:

```yaml
executor: guix-ssh
jobs: 1
guix-ssh-hosts:
  - 2325k55.local:22:/home/user/.cache/snakemake-guix-worker
